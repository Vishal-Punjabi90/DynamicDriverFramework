...
public class POST_Class_3 {
	public static void execute() throws IOException {
		...
		for (int i=0; i<5; i++)
		{				
			int statusCode=APICommonFunction.statusCode(PostReqRepository.baseURI(), PostReqRepository.resource(), PostReqRepository.requestBody3());
		
		if (statusCode==201)
		{
			String responseBody =APICommonFunction.responseBody(PostReqRepository.baseURI(), PostReqRepository.resource(), PostReqRepository.requestBody3());
			System.out.println(responseBody);
			
		POST_Class_3.validator(statusCode, responseBody);
		Utilitycommonfunction.evidencefilecreator("Sample3", PostReqRepository.requestBody3(), responseBody);
		break;
		}
		...
		JsonPath jspreq=new JsonPath(PostReqRepository.requestBody3());
